    ...
    public class ClientController : BaseController
{
    ...
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<Client>> Post(int id, [FromBody] ClientDto auditoriaDto)
    {
        var auditoria = _mapper.Map<Client>(auditoriaDto);
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        // public ActionResult<ClientDto> (int id, [FromBody] ClientDto ClientDto)
        // {
    ...
    {
        var auditoria = await _unitOfWork.Cities.GetAllAsync();

        if (auditoria == null)
            return BadRequest();

        _unitOfWork.Cities.Remove(ClientErrorData);
        await _unitOfWork.SaveAsync();
        return NoContent();
    }
}}